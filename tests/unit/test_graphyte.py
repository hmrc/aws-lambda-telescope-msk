...
from telescope_msk.graphyte import Sender
...
class TestSender(Sender):
    ...
    def __init__(self, *args, **kwargs):
        Sender.__init__(self, "dummy_host", *args, **kwargs)
        self.messages = []
...
class TestBuildMessage(unittest.TestCase):
    def test_no_prefix(self):
        sender = TestSender()
        self.assertEqual(
            sender.build_message("foo.bar", 42, 12345), b"foo.bar 42 12345\n"
        )
        self.assertEqual(
            sender.build_message("boo.far", 42.1, 12345.6), b"boo.far 42.1 12346\n"
        )

    def test_unicode(self):
        sender = TestSender()
        self.assertEqual(
            sender.build_message("\u201cfoo.bar\u201d", 42, 12345),
    ...
    def test_prefix(self):
        ...
        self.assertEqual(
            sender.build_message("boo.far", 567, 12347), b"pr.efix.boo.far 567 12347\n"
        )
    ...
    def test_tagging_none(self):
        sender = TestSender()
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345), b"tag.test 42 12345\n"
        )

    def test_tagging_single(self):
        sender = TestSender()
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345, {"foo": "bar"}),
    ...
    def test_tagging_numeric_value(self):
        sender = TestSender()
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345, {"foo": 123}),
    ...
    def test_tagging_unicode(self):
        sender = TestSender()
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345, {"\u201cfoo.bar\u201d": 123}),
    ...
    def test_tagging_multi(self):
        sender = TestSender()
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345, {"foo": "bar", "ding": "dong"}),
    ...
    def test_tagging_default(self):
        ...
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345), b"tag.test;foo=bar 42 12345\n"
        )

    def test_tagging_override(self):
        sender = TestSender(tags={"foo": "bar", "ding": "dong"})
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345, {"foo": "not-bar"}),
    ...
    def test_tagging_default_no_overlap(self):
        sender = TestSender(tags={"foo": "bar"})
        self.assertEqual(
            sender.build_message("tag.test", 42, 12345, {"ding": "dong"}),
            b"tag.test;ding=dong;foo=bar 42 12345\n",
        )
...
class TestSynchronous(unittest.TestCase):
    ...
    def test_sender_disallow_interval_and_raise_send_errors(self):
        with self.assertRaises(ValueError):
            Sender("test", interval=1, raise_send_errors=True)
...
class TestSendSocketTCP(unittest.TestCase):
    ...
    def test_send_socket(self):
        init("127.0.0.1")
        send("foo", 42, timestamp=12345)
        send("bar", 43.5, timestamp=12346)
...
class TestSendSocketUDP(unittest.TestCase):
    ...
    def test_send_socket(self):
        init("127.0.0.1", protocol="udp")
        send("foo", 42, timestamp=456789)
        send("bar", 43.5, timestamp=456789)
        self.server.handle_request()
        self.server.handle_request()
        self.assertEqual(TestHandler.pop_message(), b"foo 42 456789\n")
        self.assertEqual(TestHandler.pop_message(), b"bar 43.5 456789\n")
...
class TestInterval(unittest.TestCase):
    ...
    def test_send_multiple(self):
        ...
        time.sleep(0.2)
        self.assertEqual(
            self.sender.pop_message(), b"foo 42 12345\nbar 43 12346\nbaz 44 12347\n"
        )
...
class TestIntervalBatch(unittest.TestCase):
    ...
    def test_send_many_multiple(self):
        ...
        time.sleep(0.2)
        self.assertEqual(
            self.sender.pop_message(),
            b"foo 42 12345\nbar 43 12346\nbaz 44 12347\nbaz 45 12348\nbaz 46 12349\n",
        )
        self.assertEqual(self.sender.pop_message(), b"baz 47 12350\n")
