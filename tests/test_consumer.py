...
from srctelescope_msk.consumer import get_consumer, get_metrics_for_group_and_topic, get_metrics_for_topic, \
...
class GetMetricsForGroupsAndTopics:
    def test_calls_get_metrics_for_group_and_topic_for_each_key_value(self):
        with patch('srctelescope_msk.consumer.get_metrics_for_group_and_topic') as mock_get_metrics:
...
class GetMetricsForGroupAndTopic(TestCase):
    def test_creates_consumer_on_group(self):
        ...
        with patch('srctelescope_msk.consumer.get_consumer') as mock_get_consumer:
    ...
    def test_calls_get_metrics_for_topic(self):
        ...
        with patch('srctelescope_msk.consumer.get_consumer') as mock_get_consumer:
            ...
            with patch('srctelescope_msk.consumer.get_metrics_for_topic') as get_metrics:
...
class GetPartitionsForTopic(TestCase):
    ...
    def test_TopicPartitions_are_created(self):
        ...
        with patch('srctelescope_msk.consumer.TopicPartition') as mockTopicPartition:
...
class GetPartitionsForTopics(TestCase):
    def test_calls_through_for_each_topic(self):
        with patch('srctelescope_msk.consumer.get_partitions_for_topic') as get_partitions:
...
class GetMetricsForPartitions(TestCase):
    def test_calls_through_for_each_partition(self):
        consumer = MagicMock()
        with patch('srctelescope_msk.consumer.get_metrics_for_partition') as get_metrics:
