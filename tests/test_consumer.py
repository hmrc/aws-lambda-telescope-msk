...
from telemetry.telescope_msk.consumer import get_consumer, get_metrics_for_group_and_topic, get_metrics_for_topic, \
    get_metrics_for_partition, get_metrics_for_partitions, get_partitions_for_topic, get_partitions_for_topics, \
...
class GetMetricsForGroupsAndTopics:
    def test_calls_get_metrics_for_group_and_topic_for_each_key_value(self):
        with patch('telemetry.telescope_msk.consumer.get_metrics_for_group_and_topic') as mock_get_metrics:
...
class GetMetricsForGroupAndTopic(TestCase):
    def test_creates_consumer_on_group(self):
        ...
        with patch('telemetry.telescope_msk.consumer.get_consumer') as mock_get_consumer:
    ...
    def test_calls_get_metrics_for_topic(self):
        ...
        with patch('telemetry.telescope_msk.consumer.get_consumer') as mock_get_consumer:
            ...
            with patch('telemetry.telescope_msk.consumer.get_metrics_for_topic') as get_metrics:
                get_metrics.return_value = [{
                    'metrics': 'foo'
                }]
                out = get_metrics_for_group_and_topic('bootstrap_servers', 'group', 'topic')
...
class GetPartitionsForTopic(TestCase):
    ...
    def test_TopicPartitions_are_created(self):
        ...
        with patch('telemetry.telescope_msk.consumer.TopicPartition') as mockTopicPartition:

            test_topic = 'test_topic_01'
            mock_partition = 6

            get_partitions_for_topic(Mock(error=None, topic=test_topic, partitions=[mock_partition]))
...
class GetPartitionsForTopics(TestCase):
    def test_calls_through_for_each_topic(self):
        with patch('telemetry.telescope_msk.consumer.get_partitions_for_topic') as get_partitions:
            get_partitions.return_value = []
            mock_topics = {
                'test_topic_1': Mock(error='error1', topic="test_topic_1", partitions=[1]),
                'test_topic_2': Mock(error='error2', topic="test_topic_2", partitions=[2])
            }
...
class GetMetricsForPartitions(TestCase):
    def test_calls_through_for_each_partition(self):
        consumer = MagicMock()
        with patch('telemetry.telescope_msk.consumer.get_metrics_for_partition') as get_metrics:
...
class GetMetricsForPartition(TestCase):
    ...
    def test_negative_partition_offset(self):
        ...
        self.assertEqual(get_metrics_for_partition(consumer, Mock(offset=-1, partition=11, topic='test_topic')), {
    ...
    def test_returns_normal_lag(self):
        ...
        self.assertEqual(get_metrics_for_partition(consumer, Mock(offset=0, partition=11, topic='test_topic')), {
    ...
    def test_raises_exception_on_None(self):
        ...
        l = lambda _ : get_metrics_for_partition(consumer, Mock(offset=-1, partition=11, topic='test_topic'))
