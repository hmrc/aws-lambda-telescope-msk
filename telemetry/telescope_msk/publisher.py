...
def publish_metric_sums(metrics: list, graphite_host: str):
    ...
    for topic_name in sums:
        ...
        publish_kafka_to_graphite(f"{topic_name}.sum-high", topic_sum['sum_high'], graphite_host)
        publish_kafka_to_graphite(f"{topic_name}.sum-low", topic_sum['sum_low'], graphite_host)
        publish_kafka_to_graphite(f"{topic_name}.sum-lag", topic_sum['sum_lag'], graphite_host)
