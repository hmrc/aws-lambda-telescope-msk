...
def get_metrics_for_groups_and_topics(bootstrap_servers: str, consumer_groups_topic_names: dict) -> list:
    metrics = []
    for group_name in consumer_groups_topic_names:
        topic_name = consumer_groups_topic_names[group_name]
        metrics += get_metrics_for_group_and_topic(bootstrap_servers, group_name, topic_name)
    return metrics
